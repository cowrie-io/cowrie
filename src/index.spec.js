...
describe('Handles integer & floating point arithmetcis', () => {
    describe('Adds a given set of numbers to the provided money', () => {
        const cases = [
            {input: [0.1, 0.2], output: `0.30`},
            {input: [1.1, 1.1, 1.1, 4], output: `7.30`},
        ];

        cases.forEach((variation) => {
            const {input, output} = variation;
            test(`add ${JSON.stringify(input)} to get ${output}`, () => {
                expect(new Cowrie().plus(...input).figure).toBe(output);
            });
        });
    });
